...
**EduAssist** is a native iOS/iPadOS SwiftUI app for K-12 education. Firebase is the entire backend (Auth, Firestore, Cloud Functions). Codex AI powers the student companion chat via a Cloud Function — the API key is never in the iOS app.
...
3. Calls `Codex-sonnet-4-6` via `@anthropic-ai/sdk` (API key via Secret Manager)
...
All classifiers run synchronously before Codex is called; latency target < 100 ms each.

| Stage | FR | Behavior |
|---|---|---|
| Input classifier | FR-100 | BLOCKED (violence, weapons, drugs, sexual) or NEEDS_REVIEW (bullying, distress) — rejects before Codex |
...
| Output classifier | FR-101 | Blocks harmful instructions in Codex's reply; flags opinions/politics |
